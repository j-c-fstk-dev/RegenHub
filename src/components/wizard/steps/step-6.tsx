...
const Step6 = () => {
    ...
    const form = useForm<Step6FormValues>({
        ...
        defaultValues: {
            mediaUrls: draft?.mediaUrls?.length ? draft.mediaUrls : [{ url: "" }],
        },
    });
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file || !storage || !draft?.id) return;

        setIsUploading(true);
        try {
            const filePath = `actions/${draft.id}/${Date.now()}-${file.name}`;
            const storageRef = ref(storage, filePath);
            const uploadResult = await uploadBytes(storageRef, file);
            const downloadURL = await getDownloadURL(uploadResult.ref);
            
            // Find the first empty URL field to replace, or append a new one
